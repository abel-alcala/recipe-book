...
router.post("/login", (req, res) => {
  ...
  if (!username || !password) {
    ...
    import_credential_svc.default.verify(username, password).then((goodUser) => generateAccessToken(goodUser)).then((token) => res.status(200).send({ token })).catch((_) => res.status(401).send("Unauthorized"));
  }
});
